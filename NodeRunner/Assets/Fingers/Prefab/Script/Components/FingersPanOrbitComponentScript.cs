...
namespace DigitalRubyShared
{
    ...
    public class FingersPanOrbitComponentScript : MonoBehaviour
    {
        ...
        //[Range(0.0f, 360.0f)]
        public Vector2 OrbitXMaxDegrees = new Vector2(0.0f, 0f);
        ...
        {
            if (CakeDecorationManager.IsPointerOverUIObject())
                return;
        ...
        private void UpdateOrbit(float xVelocity, float yVelocity)
        {
            ...
            if (OrbitXSpeed != 0.0f && yVelocity != 0.0f)
            {
                ...
                else
                {
                    ...
                    if (OrbitXMaxDegrees.magnitude > 0.0f)
                    {
                        float newDegrees = xDegrees + addAngle;
                        if (newDegrees > OrbitXMaxDegrees.x)
                        {
                            addAngle = OrbitXMaxDegrees.x - xDegrees;
                        }
                        else if (newDegrees < OrbitXMaxDegrees.y)
                        {
                            addAngle = OrbitXMaxDegrees.y - xDegrees;
                        }
                    }
                    xDegrees += addAngle;
